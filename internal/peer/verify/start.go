...
func Verify(channelID string) {
	logger.Debugf("Starting Verify : %s")
	verifyLedger := pInstance.GetLedger(channelID)
	blockchainInfo, err := verifyLedger.GetBlockchainInfo()
	if err != nil {
	...
	signer := mgmt.GetLocalSigningIdentityOrPanic(factory.GetDefault())
	policyMgr := policies.PolicyManagerGetterFunc(pInstance.GetPolicyManager)

	mcs := peergossip.NewMCS(
		policyMgr,
		signer,
		mgmt.NewDeserializersManager(factory.GetDefault()),
		factory.GetDefault())
